...
void CAquarium::Erased(std::shared_ptr<CItem> item)
{
...
/**
 * Will compare the fish in the list if is different then guilman
    get their locations
    and calculate distances
 * \param item 
 * \returns boolean
 */
bool CAquarium::Afraid(CItem* item)
{
    for (std::shared_ptr<CItem> fish : mItems) {
        ...
        if (fish.get() != item)
        {
            ...
            double dx = fishX - item->GetX();
            double dy = fishY - item->GetY();
