...
namespace Testing
{
	...
	public:
		...
		TEST_METHOD(TestCPictureConstructor)
		{
			CPicture picture;
		...
		TEST_METHOD(TestCPictureSize)
		{
			...
			Assert::IsTrue(picture.GetSize().Equals(Size(800, 600)));

			picture.SetSize(Size(123, 456));

			Assert::IsTrue(picture.GetSize().Equals(Size(123, 456)));

			CPicture picture2;

			Assert::AreEqual(800, picture2.GetSize().Width);
			Assert::AreEqual(600, picture2.GetSize().Height);

			picture2.SetSize(Size(123, 456));

			Assert::AreEqual(123, picture2.GetSize().Width);

			Assert::AreEqual(456, picture2.GetSize().Height);
