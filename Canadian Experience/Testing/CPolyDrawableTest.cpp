...
namespace Testing
{
	...
	public:
		...
		TEST_METHOD(TestCPolyDrawableColor)
		{
			...
			draw.SetColor(Color::Aqua);


			Assert::IsTrue(CompareColors(Color::Aqua, draw.GetColor()));
		}
		...
		* of the rotation of a drawable works */
		TEST_METHOD(TestCPolyDrawableAnimation)
		{
			...
			// Test beTweening the two keyframs
			drawable->SetRotation(0.3);
			picture->SetAnimationTime(2.25);    // Halfway beTweening the two keyframes
			Assert::AreEqual((2.7 + -1.8) / 2, drawable->GetRotation(), 0.00001);

			drawable->SetRotation(0.3);
			picture->SetAnimationTime(2.0);    // 1/3 beTweening the two keyframes
