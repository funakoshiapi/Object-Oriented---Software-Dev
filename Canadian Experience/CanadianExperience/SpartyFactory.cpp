...
std::shared_ptr<CActor> CSpartyFactory::Create()
{
	...
	auto headb = make_shared<CImageDrawable>(L"Head Bottom", L"images/sparty_headb.png");
	headb->SetCenter(Point(53, 30));
	headb->SetPosition(Point(0, 37 - 144));
	torso->AddChild(headb);

	auto headt = make_shared<CHeadTop>(L"Head Top", L"images/sparty_headt.png");
	headt->SetCenter(Point(59, 143));
	headt->SetPosition(Point(9, -24));
